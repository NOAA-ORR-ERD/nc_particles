...
pytestmark = pytest.mark.skip("tests need to be updated")

HERE = Path(__file__).parent

# test the writer...
def test_writer_with_ref_time():
    w = nc_particles.Writer(HERE / 'junk_file.nc', ref_time=datetime.datetime(2010, 2, 3, 0))
    del w
    nc = netCDF4.Dataset(HERE / 'junk_file.nc')
...
def test_write_timestep():
    """very simple version"""
    w = nc_particles.Writer(HERE / 'junk_file.nc',
